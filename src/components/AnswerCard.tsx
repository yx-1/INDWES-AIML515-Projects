...
export const AnswerCard: React.FC<AnswerCardProps> = ({ record, onFeedback }) => {
  ...
  const handleRatingClick = async (rating: 'helpful' | 'not_helpful') => {
    ...
    try {
      ...
      setFeedbackError("Firestore failure: Could not save feedback. Please try again.");
    } finally {
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    if (tempRating) {
      setIsSavingFeedback(true);
      try {
        await onFeedback(record.id, tempRating, feedbackComment);
        setShowCommentBox(false);
      } catch (err: any) {
        setFeedbackError("Firestore failure: Could not save feedback comment.");
      } finally {
        setIsSavingFeedback(false);
      }
    }
  ...
  return (
    ...
    >
      <div className="p-6 flex-1 space-y-6">
        ...
        {/* Geometric Lead: Blue rounded badge + Main Guidance */}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-1.5 flex items-center gap-1.5 text-[11px] text-slate-600">
                          <span className="text-slate-400 font-semibold text-[10px] uppercase tracking-wide">Source URL:</span>
                          {src.urlOrDocRef.startsWith('http') ? (
                            <a
                              href={src.urlOrDocRef}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="text-blue-600 font-medium hover:underline truncate max-w-[340px] flex items-center gap-1"
      ...
      {/* Bottom Bar: Permitting Authority Disclaimer & Feedback */}
        ...
        <form onSubmit={handleCommentSubmit} className="p-3 bg-slate-100/80 border-t border-slate-200">
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 rounded-md border border-slate-300 bg-white px-3 py-1.5 text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
            <button
              type="submit"
              disabled={isSavingFeedback}
